...
2. **`batchrun.md`** (root) — suite-wide scheduled / batch jobs and the status of their most recent runs
3. **`ai_specs/overview.md`** — the whole stack's purpose, the packages, and who it is for
4. **`ai_specs/architecture-planning.md`** — THE layering: the packages, their order, and the dependency direction
5. **`ai_specs/design.md`** — shared design instructions, specifications, patterns, and non-functional requirements
6. **`ai_specs/domain-knowledge.md`** — shared terminology and business rules
7. **`ai_specs/llm-integration.md`** — shared model choice and guardrails
8. **`ai_specs/conventions.md`** — one shared coding / git / testing standard for every package
9. **`ai_specs/auth-security.md`** — shared identity model and security posture
...
- **Tier 1 — Root (this folder):** shared specs every package inherits. Operational files (`CLAUDE.md`, `project_progress.md`, `batchrun.md`) sit **loose** at the root; specs live inside `ai_specs/`.
...
`CLAUDE.md`, the progress files, and `batchrun.md` are **operational** — they must sit at a folder root and *point into* `ai_specs/`. Never put them inside `ai_specs/`. There is **one** `batchrun.md` for the whole suite; it lives at the root, never inside a package.
...
- Whenever you add or change a scheduled / batch job anywhere in the suite, update the root `batchrun.md` (name the owning package).
